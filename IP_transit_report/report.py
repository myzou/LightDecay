...
def readExcel(filename):
    ...
    for i in range(1, ws.max_row + 1):
        ...
        if (bandwidth != None and device != None and interface != None and str(bandwidth).isdigit()):
            try:
                ...
                result = getResult(bandwidth=str(bandwidth).strip(), device=str(device).strip(), interface=str(interface).strip())
            except Exception as e:
                # print(e)
                continue
    ...
    ws.cell(row=1, column=2, value=str(param.startTime+"——"+param.endTime))
    wb.save(filename)
    wb.close()
